...
class Role_List_Table extends \WP_List_Table {
	...
	public function prepare_items() {
		...
		$roles = apply_filters( 'memberscontrol_manage_roles_items', $roles, $this->role_view );
	...
	protected function column_default( $role, $column_name ) {
		...
		return apply_filters( "memberscontrol_manage_roles_column_{$column_name}", '', $role );
	}
	...
	protected function column_cb( $role ) {
		...
		if ( $role == get_option( 'default_role' ) || in_array( $role, $this->current_user->roles ) || ! memberscontrol_is_role_editable( $role ) )
			$out = '';

		else
			$out = sprintf( '<input type="checkbox" name="roles[%1$s]" value="%1$s" />', esc_attr( $role ) );

		return apply_filters( 'memberscontrol_manage_roles_column_cb', $out, $role );
	}
	...
	protected function column_title( $role ) {
		...
		$states = apply_filters( 'memberscontrol_role_states', $states, $role );
		...
		if ( current_user_can( 'edit_roles' ) )
			$title = sprintf( '<strong><a class="row-title" href="%s">%s</a>%s</strong>', esc_url( memberscontrol_get_edit_role_url( $role ) ), esc_html( memberscontrol_get_role( $role )->get( 'label' ) ), $role_states );

		else
			$title = sprintf( '<strong><span class="row-title">%s</span>%s</strong>', esc_html( memberscontrol_get_role( $role )->get( 'label' ) ), $role_states );

		return apply_filters( 'memberscontrol_manage_roles_column_role_name', $title, $role );
	}
	...
	protected function column_role( $role ) {
		return apply_filters( 'memberscontrol_manage_roles_column_role', memberscontrol_sanitize_role( $role ), $role );
	}
	...
	protected function column_users( $role ) {
		...
		$user_count = memberscontrol_get_role_user_count( $role );
		...
		return apply_filters( 'memberscontrol_manage_roles_column_users', $output, $role );
	}
	...
	protected function column_granted_caps( $role ) {
		return apply_filters( 'memberscontrol_manage_roles_column_granted_caps', memberscontrol_get_role_granted_cap_count( $role ), $role );
	}
	...
	protected function column_denied_caps( $role ) {
		return apply_filters( 'memberscontrol_manage_roles_column_denied_caps', memberscontrol_get_role_denied_cap_count( $role ), $role );
	}
	...
	protected function handle_row_actions( $role, $column_name, $primary ) {
		...
		if ( $primary === $column_name ) {
			...
			if ( memberscontrol_is_role_editable( $role ) ) {

				// If the current user can edit the role, add an edit link.
				if ( current_user_can( 'edit_roles' ) )
					$actions['edit'] = sprintf( '<a href="%s">%s</a>', esc_url( memberscontrol_get_edit_role_url( $role ) ), esc_html__( 'Edit', 'memberscontrol' ) );
				...
				if ( ( is_multisite() && is_super_admin() && $role !== $this->default_role ) || ( current_user_can( 'delete_roles' ) && $role !== $this->default_role && ! current_user_can( $role ) ) )
					$actions['delete'] = sprintf( '<a class="memberscontrol-delete-role-link" href="%s">%s</a>', esc_url( memberscontrol_get_delete_role_url( $role ) ), esc_html__( 'Delete', 'memberscontrol' ) );

			// If the role cannot be edited.
			} elseif ( current_user_can( 'edit_roles' ) ) {

				// Add the view role link.
				$actions['view'] = sprintf( '<a href="%s">%s</a>', esc_url( memberscontrol_get_edit_role_url( $role ) ), esc_html__( 'View', 'memberscontrol' ) );
			}

			// If the current user can create roles, add the clone role link.
			if ( current_user_can( 'create_roles' ) )
				$actions['clone'] = sprintf( '<a href="%s">%s</a>', esc_url( memberscontrol_get_clone_role_url( $role ) ), esc_html__( 'Clone', 'memberscontrol' ) );
			...
			if ( current_user_can( 'manage_options' ) && $role === $this->default_role )
				$actions['default_role'] = sprintf( '<a href="%s">%s</a>', esc_url( admin_url( 'options-general.php#default_role' ) ), esc_html__( 'Change Default', 'memberscontrol' ) );

			// If the currrent user can view users, add a users link.
			if ( current_user_can( 'list_users' ) )
				$actions['users'] = sprintf( '<a href="%s">%s</a>', memberscontrol_get_role_users_url( $role ), esc_html__( 'Users', 'memberscontrol' ) );

			// Allow devs to filter the row actions.
			$actions = apply_filters( 'memberscontrol_roles_row_actions', $actions, $role );
		}
	...
	protected function get_views() {
		...
		$this->role_views['all'] = array(
			'label_count' => _n_noop( 'All %s', 'All %s', 'memberscontrol' ),
			'roles'       => array_keys( memberscontrol_get_roles() )
		);

		$this->role_views['mine'] = array(
			'label_count' => _n_noop( 'Mine %s', 'Mine %s', 'memberscontrol' ),
			'roles'       => $current_user->roles
		);

		$this->role_views['active'] = array(
			'label_count' => _n_noop( 'Has Users %s', 'Has Users %s', 'memberscontrol' ),
			'roles'       => memberscontrol_get_active_roles()
		);

		$this->role_views['inactive'] = array(
			'label_count' => _n_noop( 'No Users %s', 'No Users %s', 'memberscontrol' ),
			'roles'       =>memberscontrol_get_inactive_roles()
		);

		$this->role_views['editable'] = array(
			'label_count' => _n_noop( 'Editable %s', 'Editable %s', 'memberscontrol' ),
			'roles'       => memberscontrol_get_editable_roles()
		);

		$this->role_views['uneditable'] = array(
			'label_count' => _n_noop( 'Uneditable %s', 'Uneditable %s', 'memberscontrol' ),
			'roles'       => memberscontrol_get_uneditable_roles()
		);

		// Loop through the role groups and put them into the view list.
		foreach ( memberscontrol_get_role_groups() as $group ) {
		...
		foreach ( $this->role_views as $view => $args ) {
			...
			$views[ $view ] = sprintf(
				...
				'all' === $view ? esc_url( memberscontrol_get_edit_roles_url() ) : esc_url( memberscontrol_get_role_view_url( $view ) ),
				sprintf( translate_nooped_plural( $noop, $count, $noop['domain'] ), sprintf( '<span class="count">(%s)</span>', number_format_i18n( $count ) ) )
			);
		}

		return apply_filters( 'memberscontrol_manage_roles_views', $views, $this->role_view, memberscontrol_get_edit_roles_url() );
	}
	...
	protected function get_bulk_actions() {
		...
		if ( current_user_can( 'delete_roles' ) )
			$actions['delete'] = esc_html__( 'Delete', 'memberscontrol' );

		return apply_filters( 'memberscontrol_manage_roles_bulk_actions', $actions );
	}
}
