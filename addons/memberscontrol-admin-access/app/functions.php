...
function get_default_settings() {
	...
	$settings = [
		'roles'           => array_keys( memberscontrol_get_roles() ), // Defaults to all roles.
...
function get_setting( $option = '' ) {
	...
	$settings = wp_parse_args( get_option( 'memberscontrol_admin_access_settings', $defaults ), $defaults );
...
function user_has_access( $user_id = 0 ) {
	...
	return apply_filters(
		...
		memberscontrol_user_has_role( $user_id, get_roles_with_access() ),
		$user_id
	);
}
