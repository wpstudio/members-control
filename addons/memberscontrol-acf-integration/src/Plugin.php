...
use function memberscontrol_register_cap_group;
use function memberscontrol_register_role_group;
use function memberscontrol_unregister_cap_group;
...
class Plugin {
	...
	public function load() {
		...
		add_action( 'memberscontrol_register_role_groups', [ $this, 'registerRoleGroups' ] );
		add_action( 'memberscontrol_register_cap_groups',  [ $this, 'registerCapGroups'  ] );
	...
	public function loadTextdomain() {
		...
		load_plugin_textdomain(
			'memberscontrol-acf-integration',
			false,
	...
	public function registerRoleGroups() {
		...
		if ( $roles ) {
			members_register_role_group( 'plugin-acf', [
				'label'       => esc_html__( 'Advanced Custom Fields', 'memberscontrol-acf-integration' ),
				'label_count' => _n_noop( 'Advanced Custom Fields %s', 'Advanced Custom Fields %s', 'memberscontrol-acf-integration' ),
				'roles'       => $roles,
			] );
		}
	}
	...
	public function registerCapGroups() {
		...
		if ( $type = get_post_type_object( 'acf-field-group' ) ) {
			...
			foreach ( $groups as $group ) {
				memberscontrol_unregister_cap_group( "type-{$group}" );
			}

			// Register a cap group for the ACF plugin.
			memberscontrol_register_cap_group( 'plugin-acf', [
				'label'    => esc_html__( 'Custom Fields', 'memberscontrol-acf-integration' ),
	...
	public function registerCaps() {
		...
		foreach ( acf_caps() as $name => $options ) {
			...
			memberscontrol_register_cap( $name, [
				'label'       => $options['label'],
